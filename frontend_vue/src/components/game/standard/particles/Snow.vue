...
const createSnowflakeAnimation = (snowflake: Snowflake): void => {
  ...
  @keyframes fall-${snowflake.id} {
    ...
    25% {
      transform: translate(${snowflake.horizontalMovement * 0.25}px, ${maxHeight.value * 0.25}px) 
                 rotate(${Math.random() * 90}deg);
      opacity: ${snowflake.opacity * 0.95};
    }
    50% {
      transform: translate(${snowflake.horizontalMovement * 0.5}px, ${maxHeight.value * 0.5}px) 
                 rotate(${Math.random() * 180}deg);
      opacity: ${snowflake.opacity * 0.9};
    }
    75% {
      transform: translate(${snowflake.horizontalMovement * 0.75}px, ${maxHeight.value * 0.75}px) 
                 rotate(${Math.random() * 270}deg);
      opacity: ${snowflake.opacity * 0.8};
    }
    100% {
      transform: translate(${snowflake.horizontalMovement}px, ${maxHeight.value}px) 
